...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadCategories() {
    fetch("/api/cardset-categories", { method: "GET" })
      ...
      .then((data) => {
        ...
        homeCategoryList &&
          createCategoryElements(data, "div", homeCategoryList, "category");
      });
  }
    ...
    cardSetCreationDiv
      ...
      .addEventListener("submit", (event) => {
        ...
        const isOnlyEnglishRegEx =
          /^[a-z0-9!" #\$%&'\(\)\*\+,-\./:;<=>\?@\[\\\]\^_`\{\|\}~]*$/i;
        if (
          cardSetCreationDiv.querySelector("#option-1").checked &&
  ...
  function changeBackground() {
    const backgroundUrl =
      ...
      "url(/static/images/background_default.jpg)";
