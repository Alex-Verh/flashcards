...
document.addEventListener("DOMContentLoaded", () => {
  const cardsetId = +document.querySelector("h1[data-id]").dataset.id;
  ...
  function loadFlashCards(cardSetId) {
    ...
    sentinel.innerHTML =
      '<img id="loading-gif" src="{{url_for("static", filename="images/loading.gif")}}" width="70vh">';
          ...
          <div class="flash-screen">
            <div class="flash-card-front">
              ${getFlashCardSideHTML(
                flashcard.title,
                flashcard.attachments.frontside
              )}
            </div>
            <div class="flash-card-back">
              ${getFlashCardSideHTML(
                flashcard.content,
                flashcard.attachments.backside
              )}
            </div>
          </div>
          `;
  ...
  const onImageRemove = (removeButton, flashCardSide, flashCardAttachments) => {
    ...
    const imageIndex = [...elToRemove.parentElement.children].indexOf(
      elToRemove
    );
