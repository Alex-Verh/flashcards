...
def create_flashcards(db, quantity=10000):
    ...
    for _ in range(quantity):
        ...
        attachments = {
            "frontside": {
                "images": [fake.file_name(category='image', extension='jpg') for _ in range(random.randint(0, 2))],
                "audio": random.choices([None, fake.file_name(category='audio', extension='mp3')], [80, 20])[0]
                },
            
            "backside": {
                "images": [fake.file_name(category='image', extension='jpg') for _ in range(random.randint(0, 3))],
                "audio": random.choices([None, fake.file_name(category='audio', extension='mp3')], [80, 20])[0]
                }
            }
